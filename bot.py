...
def generate_admin_dashboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📊 Open Admin Dashboard", web_app=WebAppInfo(url=f"{WEBAPP_URL}/admin-panel"))],
        [
...
async def execute_verification(request: Request):
    ...
    # ── IP COOLDOWN ─────────────────────────────────────────────────────────
    # አንድ IP ላይ NEW verification ካለፈ ጊዜ ብቻ 3 ደቂቃ ይጠብቅ።
    # ይህ rapid multi-account creation ለማቆም ነው (ለምሳሌ bot script
    # 10 accounts በ10 ሰከንድ ውስጥ ለመፍጠር ቢሞክር)።
    # fingerprint/clone checks ግን ሁልጊዜ ይሰራሉ — cooldown ተጨማሪ layer ብቻ ነው።
    on_cooldown, seconds_left = await ip_cooldown.is_on_cooldown(client_ip)
    if on_cooldown:
        logger.warning(f"[IP-COOLDOWN] ip={client_ip} uid={uid} remaining={seconds_left}s")
        await DataEngine.log_fraud_attempt(
...
<style>
  :root{--bg:#0B0E11;--panel:#13171C;--line:#22272E;--text:#E9E4D8;--muted:#8A8F98;--gold:#D4A24E;--red:#E0584F;}
  *{box-sizing:border-box;}
  body{margin:0;min-height:100vh;display:flex;align-items:center;justify-content:center;
    background:var(--bg);font-family:'Inter',system-ui,sans-serif;color:var(--text);}
  .card{background:var(--panel);border:1px solid var(--line);border-radius:14px;padding:40px 36px;
    width:340px;box-shadow:0 20px 60px rgba(0,0,0,.5);}
  h1{font-size:20px;margin:0 0 4px;letter-spacing:.02em;}
  p{color:var(--muted);font-size:13px;margin:0 0 24px;}
  input{width:100%;padding:12px 14px;border-radius:8px;border:1px solid var(--line);
    background:#0E1115;color:var(--text);font-size:14px;outline:none;}
  input:focus{border-color:var(--gold);}
  button{width:100%;margin-top:16px;padding:12px;border:none;border-radius:8px;
    background:var(--gold);color:#1A1300;font-weight:600;font-size:14px;cursor:pointer;}
...
@api_platform.get("/admin/login", response_class=HTMLResponse)
async def admin_login_page():
    return HTMLResponse(content=ADMIN_LOGIN_HTML)

@api_platform.post("/admin/login")
...
# ─────────────────────────────────────────────────────────────────────────────
@api_platform.get("/admin", response_class=HTMLResponse)
async def admin_dashboard_page(request: Request):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"admin.html missing: {e}")

# ─────────────────────────────────────────────────────────────────────────────
# WEB ADMIN DASHBOARD — JSON API
# ─────────────────────────────────────────────────────────────────────────────
@api_platform.get("/admin/api/stats")
async def admin_api_stats(_=Depends(require_admin_session)):
    return JSONResponse(await DataEngine.get_bot_stats())

@api_platform.get("/admin/api/users")
async def admin_api_users(search: str = "", limit: int = 50, offset: int = 0, _=Depends(require_admin_session)):
    rows, total = await DataEngine.get_all_users(search.strip(), limit, offset)
    users = [
        {
            "user_id": r["user_id"], "username": r["username"], "full_name": r["full_name"],
            "balance": float(r["balance"]), "is_banned": bool(r["is_banned"]),
            "referred_by": r["referred_by"], "joined_at": r["joined_at"],
        }
        for r in rows
    ]
    return JSONResponse({"users": users, "total": total})

@api_platform.get("/admin/api/user/{uid}")
async def admin_api_user_detail(uid: int, _=Depends(require_admin_session)):
    user = await DataEngine.get_user(uid)
    if not user:
        raise HTTPException(status_code=404, detail="User not found.")
    direct, tier2 = await DataEngine.get_referral_metrics(uid)
    verif = await DataEngine.get_verification(uid)
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        cur = await db.execute(
            "SELECT reason, ip_address, details, logged_at FROM fraud_log "
            "WHERE user_id = ? ORDER BY logged_at DESC LIMIT 10", (uid,)
        )
        fraud_rows = [dict(r) for r in await cur.fetchall()]
    return JSONResponse({
        "user_id": user["user_id"], "username": user["username"], "full_name": user["full_name"],
        "balance": float(user["balance"]), "is_banned": bool(user["is_banned"]),
        "joined_at": user["joined_at"], "direct_referrals": direct, "tier2_referrals": tier2,
        "verification": {
            "ip_address": verif["ip_address"] if verif else None,
            "tg_platform": verif["tg_platform"] if verif else None,
            "tg_version": verif["tg_version"] if verif else None,
            "verified_at": verif["verified_at"] if verif else None,
        } if verif else None,
        "fraud_log": fraud_rows,
    })

@api_platform.post("/admin/api/users/{uid}/ban")
async def admin_api_ban_user(uid: int, _=Depends(require_admin_session)):
    await DataEngine.ban_user(uid, 1)
    try:
        await bot.send_message(uid, "🚫 <b>Your account has been banned from this bot.</b>")
    except Exception:
        pass
    return JSONResponse({"status": "banned", "user_id": uid})

@api_platform.post("/admin/api/users/{uid}/unban")
async def admin_api_unban_user(uid: int, request: Request, _=Depends(require_admin_session)):
    body = await request.json() if await request.body() else {}
    mode = body.get("mode", "std")
    await DataEngine.create_user(uid, "", "")
    await DataEngine.ban_user(uid, 0)
    if mode == "full":
        await DataEngine.inject_fake_verification(uid)
        msg = "🚀 You've been unbanned with direct menu access."
    else:
        await DataEngine.full_clear_verification(uid)
        msg = "✅ You've been unbanned. Please reverify via the Mini App."
    try:
        await bot.send_message(uid, msg)
    except Exception:
        pass
    return JSONResponse({"status": "unbanned", "mode": mode, "user_id": uid})

@api_platform.get("/admin/api/withdrawals")
async def admin_api_withdrawals(status: str = "pending", limit: int = 100, _=Depends(require_admin_session)):
    rows = await DataEngine.get_withdrawals_filtered(status, limit)
    items = [
        {
            "id": r["id"], "user_id": r["user_id"], "amount": float(r["amount"]),
            "full_name": r["full_name"], "phone": r["phone"], "status": r["status"],
            "reason": r["reason"], "created_at": r["created_at"], "resolved_at": r["resolved_at"],
        }
        for r in rows
    ]
    return JSONResponse({"withdrawals": items})

@api_platform.post("/admin/api/withdrawals/{wid}/approve")
async def admin_api_approve_withdrawal(wid: int, _=Depends(require_admin_session)):
    ticket = await DataEngine.get_withdrawal(wid)
    if not ticket or ticket["status"] != "pending":
        raise HTTPException(status_code=400, detail="Already processed or not found.")
    await DataEngine.update_withdrawal_status(wid, "approved", ticket["channel_post_id"])
    me = await bot.get_me()
    ...
    if PAYMENT_LOG_CHANNEL and ticket["channel_post_id"]:
        txt = (
            ...
            f"━━━━━━━━━━━━━━━━━━━━━━\n\n👤 <b>Recipient:</b> {sanitize_html(ticket['full_name'])}\n\n"
            f"💰 <b>Amount:</b> ETB {ticket['amount']:.2f}\n\n🚀 <b>Operational Registry:</b> Success ✅\n\n"
            f"━━━━━━━━━━━━━━━━━━━━━━"
        )
        try:
            await bot.send_photo(
                chat_id=PAYMENT_LOG_CHANNEL, photo=TELEBIRR_PROOF_IMAGE, caption=txt,
                reply_to_message_id=ticket["channel_post_id"], reply_markup=proof_kb,
            )
        except Exception as e:
            logger.error(f"[ADMIN-WEB] Proof photo error: {e}")
    try:
        await bot.send_message(
            ticket["user_id"],
            f"🎉 Your cashout of {ticket['amount']:.2f} Birr has been approved and sent via Telebirr!",
        ...
        pass
    return JSONResponse({"status": "approved", "id": wid})

@api_platform.post("/admin/api/withdrawals/{wid}/reject")
async def admin_api_reject_withdrawal(wid: int, request: Request, _=Depends(require_admin_session)):
    ticket = await DataEngine.get_withdrawal(wid)
    if not ticket or ticket["status"] != "pending":
        raise HTTPException(status_code=400, detail="Already processed or not found.")
    body   = await request.json() if await request.body() else {}
    reason = body.get("reason", "Violated bot usage policies.").strip() or "Violated bot usage policies."
    await DataEngine.update_withdrawal_status(wid, "rejected", ticket["channel_post_id"], reason)
    warning_notice = (
        f"❌ <b>Your Withdrawal Request has been Rejected!</b>\n\n"
        f"💰 <b>Amount:</b> <code>{ticket['amount']:.2f} Birr</code>\n"
        f"⚠️ <b>Reason:</b> <code>{sanitize_html(reason)}</code>"
    ...
    try:
        ...
        pass
    return JSONResponse({"status": "rejected", "id": wid})

@api_platform.get("/admin/api/fraud_log")
async def admin_api_fraud_log(limit: int = 50, _=Depends(require_admin_session)):
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        cur = await db.execute(
...
# ─────────────────────────────────────────────────────────────────────────────
# ADMIN MINIAPP — SERVE PAGE
# ─────────────────────────────────────────────────────────────────────────────
@api_platform.get("/admin-panel", response_class=HTMLResponse)
async def serve_admin_panel():
    try:
        ...
        raise HTTPException(status_code=500, detail=f"admin_panel.html missing: {e}")

# ─────────────────────────────────────────────────────────────────────────────
# ADMIN MINIAPP — AUTH HELPER
# Every protected endpoint receives initData via X-Init-Data header,
# validates the HMAC signature, then checks user_id is in ADMIN_IDS.
# ─────────────────────────────────────────────────────────────────────────────
def verify_admin_miniapp(request: Request) -> dict:
    init_data = request.headers.get("X-Init-Data", "")
    tg_user   = parse_telegram_webapp_handshake(init_data)
    if not tg_user or int(tg_user.get("id", 0)) not in ADMIN_IDS:
        raise HTTPException(status_code=403, detail="Not authorized.")
    return tg_user

# ─────────────────────────────────────────────────────────────────────────────
# ADMIN MINIAPP — JSON API
# ─────────────────────────────────────────────────────────────────────────────
@api_platform.get("/admin-panel/api/stats")
async def mp_stats(request: Request):
    verify_admin_miniapp(request)
    return JSONResponse(await DataEngine.get_bot_stats())

@api_platform.get("/admin-panel/api/users")
async def mp_users(request: Request, search: str = "", limit: int = 50, offset: int = 0):
    verify_admin_miniapp(request)
    rows, total = await DataEngine.get_all_users(search.strip(), limit, offset)
    return JSONResponse({
        "users": [
...
@api_platform.post("/admin-panel/api/users/{uid}/ban")
async def mp_ban(uid: int, request: Request):
    verify_admin_miniapp(request)
    await DataEngine.ban_user(uid, 1)
    try:
        await bot.send_message(uid, "🚫 <b>Your account has been banned.</b>")
    except Exception:
        pass
    return JSONResponse({"status": "banned"})

@api_platform.post("/admin-panel/api/users/{uid}/unban")
async def mp_unban(uid: int, request: Request):
    verify_admin_miniapp(request)
    body = await request.json() if await request.body() else {}
    mode = body.get("mode", "std")
    await DataEngine.create_user(uid, "", "")
    ...
    try:
        ...
        pass
    return JSONResponse({"status": "unbanned", "mode": mode})

@api_platform.get("/admin-panel/api/withdrawals")
async def mp_withdrawals(request: Request, status: str = "pending", limit: int = 100):
    verify_admin_miniapp(request)
    rows = await DataEngine.get_withdrawals_filtered(status, limit)
    return JSONResponse({
        "withdrawals": [
...
@api_platform.post("/admin-panel/api/withdrawals/{wid}/approve")
async def mp_approve(wid: int, request: Request):
    verify_admin_miniapp(request)
    ticket = await DataEngine.get_withdrawal(wid)
    if not ticket or ticket["status"] != "pending":
    ...
    if PAYMENT_LOG_CHANNEL and ticket["channel_post_id"]:
        txt = (f"━━━━━━━━━━━━━━━━━━━━━━\n\n✅ <b>WITHDRAWAL COMPLETED</b>\n\n"
               f"👤 {sanitize_html(ticket['full_name'])}\n💰 ETB {ticket['amount']:.2f}\n\n━━━━━━━━━━━━━━━━━━━━━━")
        try:
            await bot.send_photo(chat_id=PAYMENT_LOG_CHANNEL, photo=TELEBIRR_PROOF_IMAGE,
                                 caption=txt, reply_to_message_id=ticket["channel_post_id"], reply_markup=kb)
        except Exception:
            pass
    try:
...
@api_platform.post("/admin-panel/api/withdrawals/{wid}/reject")
async def mp_reject(wid: int, request: Request):
    verify_admin_miniapp(request)
    ticket = await DataEngine.get_withdrawal(wid)
    if not ticket or ticket["status"] != "pending":
        raise HTTPException(status_code=400, detail="Already processed.")
    body   = await request.json() if await request.body() else {}
    reason = body.get("reason", "Violated bot usage policies.").strip()
    await DataEngine.update_withdrawal_status(wid, "rejected", ticket["channel_post_id"], reason)
    try:
        await bot.send_message(ticket["user_id"],
            f"❌ <b>Withdrawal Rejected</b>\n💰 {ticket['amount']:.2f} Birr\n⚠️ {sanitize_html(reason)}")
    except Exception:
        pass
    return JSONResponse({"status": "rejected"})

@api_platform.get("/admin-panel/api/fraud_log")
async def mp_fraud(request: Request, limit: int = 50):
    verify_admin_miniapp(request)
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        cur = await db.execute("SELECT * FROM fraud_log ORDER BY logged_at DESC LIMIT ?", (limit,))
        rows = await cur.fetchall()
    return JSONResponse({"entries": [dict(r) for r in rows]})
